...
describe('BrowserTrackExporter', () => {
    it('routes browser and archive exports through the same background task', async () => {
        ...
        const application = {
            ...
            async runTrackDownloadSession(
                version: { sessionId: string; revision: number },
                operation: (downloadTrack: DeviceGateway['downloadTrack']) => Promise<unknown>
            ) {
                assert.deepEqual(version, { sessionId: 'session', revision: 4 });
                return operation(async (index: number, progress: (value: { read: number; total: number }) => void) => {
