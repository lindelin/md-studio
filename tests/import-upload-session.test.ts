...
describe('import upload session', () => {
    ...
    it('passes cancellation into an active device upload and finalizes the session', async () => {
        ...
        const pending = runImportUploadSession({
            tracks: tracks(track('one.wav')),
            totalTracks: 1,
            format: { codec: 'AT3', bitrate: 132 },
            disc,
            service,
        ...
        assert.deepEqual(await pending, { writtenTracks: 0, cancelled: true });
        assert.equal(finalized, true);
    });
});
