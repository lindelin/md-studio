...
FFMPEG_PREFIX=/src/build
TOOL_SHIMS=$(cd ../tool-shims && pwd)
: "${GIT_BASH:?GIT_BASH must point to Git for Windows bash.exe}"
...
build_lame(){
  ...
  LD="$TOOL_SHIMS/emcc" \
  AR="$TOOL_SHIMS/emar" \
  RANLIB="$TOOL_SHIMS/emranlib" \
  NM="$EMSDK/upstream/bin/llvm-nm.exe" \
  "$GIT_BASH" ./configure \
...
configure_ffmpeg() {
  MSYS2_ARG_CONV_EXCL='--prefix=' emconfigure "$GIT_BASH" ./configure \
    --arch=i686 \
    --target-os=linux \
    ...
    --extra-cflags="-Ibuild/include" \
    --extra-cxxflags="-Ibuild/include" \
    --extra-ldflags="-Lbuild/lib" \
    --disable-pthreads \
    --disable-doc \
    --disable-postproc \
    --disable-stripping \
...
build_ffmpegjs() {
  emcc \
    ...
    -lavdevice -lavfilter -lavformat -lavcodec -lswresample -lswscale -lavutil -lm -lz -lmp3lame \
    --closure 1 \
