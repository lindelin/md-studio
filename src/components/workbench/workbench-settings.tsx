    ...
    return (
        <section className="workbench__settings" aria-label="Application settings">
            ...
            <div className="workbench__settings-columns">
                <div>
                    <section className="workbench__settings-card"><span className="workbench__eyebrow">APPEARANCE</span><h3>Interface</h3><label className="workbench__settings-field"><span>Color theme</span><select value={settings.colorTheme} disabled={busy} onChange={(event) => void apply({ colorTheme: event.target.value as UserSettings['colorTheme'] }, 'Color theme updated.')}><option value="system">Use system theme</option><option value="dark">Dark</option><option value="light">Light</option></select></label><Toggle checked={settings.pageFullHeight} disabled={busy} label="Fill available height" description="Stretch the compatibility interface vertically." onChange={(checked) => updateBoolean('pageFullHeight', checked, 'Height preference updated.')} /><Toggle checked={settings.pageFullWidth} disabled={busy} label="Fill available width" description="Stretch the compatibility interface horizontally." onChange={(checked) => updateBoolean('pageFullWidth', checked, 'Width preference updated.')} /><Toggle checked={settings.vintageMode} disabled={busy} label="Win95 compatibility interface" description="Switch immediately to the original compact interface." onChange={(checked) => updateBoolean('vintageMode', checked, 'Interface preference updated.')} /></section>
