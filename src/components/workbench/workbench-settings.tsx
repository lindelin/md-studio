    ...
    return (
        <section className="workbench__settings" aria-label={t('Settings')}>
            ...
            <div className="workbench__settings-columns">
                <div>
                    <section className="workbench__settings-card"><span className="workbench__eyebrow">{t('APPEARANCE')}</span><h3>{t('Interface')}</h3><label className="workbench__settings-field"><span>{t('Language')}</span><select value={settings.uiLanguage} disabled={busy} onChange={(event) => { const uiLanguage = event.target.value as UserSettings['uiLanguage']; void apply({ uiLanguage }, 'Interface language updated.', resolveUiLanguage(uiLanguage)); }}><option value="system">{language === 'zh-CN' ? '跟随系统语言' : localizeJapanese(language, 'Use system language')}</option><option value="zh-CN">{t('Chinese (Simplified)')}</option><option value="ja">日本語</option><option value="en">{t('English')}</option></select></label><label className="workbench__settings-field"><span>{t('Color theme')}</span><select value={settings.colorTheme} disabled={busy} onChange={(event) => void apply({ colorTheme: event.target.value as UserSettings['colorTheme'] }, 'Color theme updated.')}><option value="system">{t('Use system theme')}</option><option value="dark">{t('Dark')}</option><option value="light">{t('Light')}</option></select></label></section>
                    <section className="workbench__settings-card"><span className="workbench__eyebrow">{t('WORKFLOW')}</span><h3>{t('Editing and notifications')}</h3><Toggle checked={settings.fullWidthSupport} disabled={busy} label={t('Full-width title editing')} description={t('Enable Hiragana, Kanji and full-width MiniDisc titles.')} onChange={(checked) => updateBoolean('fullWidthSupport', checked, 'Title editing preference updated.')} /><Toggle checked={settings.notifyWhenFinished} disabled={busy || notificationPermission === 'unsupported' || notificationPermission === 'denied'} label={t('Completion notifications')} description={t(notificationPermission === 'unsupported' ? 'Completion notifications are unavailable on this computer.' : notificationPermission === 'denied' ? 'Completion notifications are blocked. Enable them in Windows settings and restart MD Studio.' : 'Show a notification when a recording task finishes.')} onChange={(checked) => void updateCompletionNotifications(checked)} /></section>
